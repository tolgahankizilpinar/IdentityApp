...
namespace IdentityApp.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        ModelState.AddModelError("", "hatalı parola girişi.");
                    }
                }
                else
                {
        ...
        public async Task<IActionResult> ForgotPassword(string Email)
        {
            ...
            await _emailSender.SendEmailAsync(Email, "Parola Sıfırlama", $"Parolanızı yenilemek için linke <a href='http://localhost:5036{url}'>tıklayınız.</a>");

            TempData["message"] = "Eposta adresinize gönderilen link ile şifrenizi yenileyebilirsiniz.";
